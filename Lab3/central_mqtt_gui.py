  ...
  if processMove:
    ...
    if neil_msg in choices and naz_msg in choices:
      ...
      if neil_msg == naz_msg:
        ...
        result = "p1"
      client.publish("ece180d/central_gui", neil_msg + ',' + naz_msg + ',' + result, qos=1)    
    elif neil_msg == 'q' or naz_msg == 'q':
      client.publish("ece180d/central_quit", 'A user has disconnected...exiting', qos=1)
      client.loop_stop()
      client.disconnect()
    else:
      client.publish("ece180d/central_info", "One (or more) invalid response(s)! Please input a move in ", choices, " or 'q' to quit.")
